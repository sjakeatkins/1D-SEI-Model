...
def residual_detailed(t, SV, SV_dot):
    ...
    N_k_in_sei = np.zeros_like(SV_dot[SVptr['Ck sei'][j]])
    ...
    for j in range(params['Ny'] - 1):
        ...
        # # Li+ interstitial transport (without migration)
        # D_sei = params['D_SEI']
        # delta_Ck_sei = (Ck_sei_loc / eps_sei_loc
        #                   - Ck_sei_next / eps_sei_next)
        # D_eff_sei_array = D_sei * eps_sei_int ** brugg
        # D_eff_sei = np.matmul(D_eff_sei_array,Xk_sei_loc)
        # flux_gradient_sei = delta_Ck_sei * params['dyInv']

        grad_Flux_sei = np.zeros_like(SV_dot[SVptr['Ck sei'][j]])
        N_k_out_sei = np.zeros_like(SV_dot[SVptr['Ck sei'][j]])
