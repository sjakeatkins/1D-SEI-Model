...
def residual_detailed(t, SV, SV_dot):
    ...
    for j in range(params['Ny']-1):
     ...
     #       raise Exception('Please specify a valid transport model: cst or dst')
        ...
        #c_Li_I_Vo = np.ones_like(SV_dot[SVptr['Ck sei'][j]])
        #c_Li_I_o = np.dot(c_Li_I_Vo,SV[SVptr['eps sei'][0]])*np.exp(-1*ct.faraday*WE.electric_potential/ct.gas_constant/WE.T)
